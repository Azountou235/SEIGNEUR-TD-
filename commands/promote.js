...
module.exports = {
  ...
  execute: async (sock, msg, args) => {
    ...
    if (!isBotAdmin(sock, metadata)) {
      await sock.sendMessage(chatJid, { text: '🚫 Je dois être admin du groupe pour promouvoir quelqu\'un.' }, { quoted: msg });
      return;
    }
    ...
    if (!targets.length) {
      await sock.sendMessage(chatJid, {
        text: '⚠️ Écrivez le numéro devant *.promote*, ou mentionnez la personne, ou répondez à son message.\n\nExemple : .promote 23591234567',
      }, { quoted: msg });
      return;
    }
    ...
    try {
      ...
      await sock.sendMessage(chatJid, {
        text: `✅ Promu(s) admin : ${targets.map((t) => `@${t.split('@')[0]}`).join(', ')}`,
        mentions: targets,
      }, { quoted: msg });
    } catch (e) {
