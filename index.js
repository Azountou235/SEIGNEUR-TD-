...
"*SEIGNEUR TD* 🇷🇴\n\n❒ *STATUS* : `ONLINE`\n❒ *VERSION* : `1.0.0`\n❒ *SYSTEM* : `ACTIVE`\n\n*© SEIGNEUR TD*"
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for(const message of messages){
      ...
      // 🚫 ANTIEDIT - Restaurer messages édités
      if (antiEdit && message.message?.protocolMessage?.type === 14) {
        try {
          const editedKey = message.message.protocolMessage.key;
          const messageId = editedKey?.id;
          const remoteJid = message.key.remoteJid;
          const editerJid = message.key.participant || message.key.remoteJid;
          ...
          if (messageId) {
            ...
            if (cachedMsg) {
              ...
              if (antiEditMode === 'private') {
                ...
                notifyJid = remoteJid;
                await sendAntiEditNotif(sock, botPvJid, cachedMsg);
              }
              const senderJid = cachedMsg.sender;
              await sendAntiEditNotif(sock, notifyJid, cachedMsg);
              console.log('[ANTIEDIT] Message restauré de ' + senderJid + ' type=' + (cachedMsg.mediaType || 'texte'));
            }
          }
        } catch(e) {
          console.error('❌ Erreur antiedit upsert:', e.message);
        }
        continue;
      }
...
`╭─────────────────────────────╮\n  🚫  EXPULSION AUTOMATIQUE\n╰─────────────────────────────╯\n\n❖ @${statusSender.split('@')[0]}\n❖ ACTION  ·  Mention du groupe\n             dans un statut\n❖ STATUT  ·  ❌ EXPULSÉ\n\n╭─────────────────────────────╮\n   © SEIGNEUR TD\n╰─────────────────────────────╯`,
      ...
      // ═══════════════════════════════════════════════════════════
      // 🚫 ANTI-COMMANDES DE GROUPE (LISTENERS)
      // ═══════════════════════════════════════════════════════════
      
      if (isGroup && !isCommand) {
        try {
          const gs = getGroupSettings(remoteJid) || {};
          ...
          try {
            ...
            // Si admin, pas de restrictions
            if (isAdmin) return;
            
            // ANTILINK
            if (gs.antilink && (bodyLower.includes('http://') || bodyLower.includes('https://'))) {
              await sock.groupParticipantsUpdate(remoteJid, [senderJid], 'remove');
              await sock.sendMessage(remoteJid, {
                text: `🚫 *ANTILINK*\n\n@${senderNum} a envoyé un lien\n❌ Expulsé`
              });
              continue;
            }
            
            // ANTISTICKER
            if (gs.antisticker && message.message?.stickerMessage) {
              await sock.sendMessage(remoteJid, {
                text: `🚫 *ANTISTICKER*\n\n@${senderNum}: Les stickers sont interdits\n${gs.antisticker === 'kick' ? '❌ Expulsé' : '⚠️ Averti'}`
              });
              if (gs.antisticker === 'kick') {
                await sock.groupParticipantsUpdate(remoteJid, [senderJid], 'remove');
              }
              continue;
            }
            ...
            // ANTISPAM
            if (gs.antispam && body.length > 200) {
              await sock.sendMessage(remoteJid, {
                text: `🚫 *ANTISPAM*\n\n@${senderNum}: Message trop long\n❌ Expulsé`
              });
              await sock.groupParticipantsUpdate(remoteJid, [senderJid], 'remove');
              continue;
            }
            ...
            // ANTITAG
            if (gs.antitag && message.message?.extendedTextMessage?.contextInfo?.mentionedJid?.length > 0) {
              await sock.sendMessage(remoteJid, {
                text: `🚫 *ANTITAG*\n\n@${senderNum}: Les mentions sont interdites\n`
              });
              continue;
            }
            
            // ANTIIMAGE
            if (gs.antiimage && message.message?.imageMessage) {
              await sock.sendMessage(remoteJid, {
                text: `🚫 *ANTIIMAGE*\n\n@${senderNum}: Les images sont interdites\n❌ Expulsé`
              });
              await sock.groupParticipantsUpdate(remoteJid, [senderJid], 'remove');
              continue;
            }
            ...
            // ANTIVIDEO
            if (gs.antivideo && message.message?.videoMessage) {
              await sock.sendMessage(remoteJid, {
                text: `🚫 *ANTIVIDEO*\n\n@${senderNum}: Les vidéos sont interdites\n❌ Expulsé`
              });
              await sock.groupParticipantsUpdate(remoteJid, [senderJid], 'remove');
              continue;
            }
            ...
            // ANTIBOT
            if (gs.antibot && senderJid.includes('bot')) {
              await sock.sendMessage(remoteJid, {
                text: `🚫 *ANTIBOT*\n\n❌ Les bots sont interdits`
              });
              await sock.groupParticipantsUpdate(remoteJid, [senderJid], 'remove');
              continue;
            }
            ...
            console.error('[ANTI ERROR]', antiErr.message);
          }
        } catch(e) {}
      }
} catch(cmdErr) {
...
// 🔧 ANTIEDIT - Fonction de notification pour messages édités
async function sendAntiEditNotif(sock, notifyJid, cachedMsg) {
  const senderJid = cachedMsg.sender || '';
  const msgContent = cachedMsg.text && !['[Image]','[Video]','[Audio]','[Sticker]','[Document]','[Message]'].includes(cachedMsg.text) ? cachedMsg.text : '[ média ]';
  const header =
`┏━━━━━━━━━━━━━━━━┓\n   ✏️ MESSAGE ÉDITÉ\n┗━━━━━━━━━━━━━━━━┛\n\n❖ *AUTEUR* : @${senderJid.split('@')[0]}\n❖ *ORIGINAL* : '${msgContent}'\n\n*© SEIGNEUR TD*`;
  ...
  if (cachedMsg.mediaBuffer && cachedMsg.mediaBuffer.length > 100) {
    ...
    try {
      ...
      console.log('[ANTIEDIT] Erreur envoi media: ' + e.message);
    }
  }
  await sock.sendMessage(notifyJid, { text: header, mentions });
}
...
`✧ ───  ᴀʟɪᴠᴇ ᴀɴᴅ ʀᴇᴀᴅʏ ─── ✧\n _☁️ Sayonara everyone... just kidding!_ \n\n"I'm here to serve you."\n\n🕊️ Owner: SEIGNEUR TD\n⚡ Ping: ${aliveLatency}ms\n⏳ Uptime: ${upStr2}\n❄️ Version: 1.0.0\n\n📢 Notice: 𝙴𝚟𝚎𝚛𝚢 𝚍𝚎𝚙𝚕𝚘𝚢𝚖𝚎𝚗𝚝 𝚒𝚝'𝚜 𝚊𝚝 𝚢𝚘𝚞𝚛 𝚘𝚠𝚗 𝚛𝚒𝚜𝚔\n\n🌟 Repo : https://github.com/Azountou235/SEIGNEUR-TD-.git\n▰▰▰▰▰▰▰▰▱▱ ACTIVE\n─── ⋆⋅☆⋅⋆ ───\n> © SEIGNEUR TD`;
      ...
      case 'antidelete': {
        const state = args[0]?.toLowerCase();
        if (!isGroup) { await sock.sendMessage(remoteJid, { text: '❌ Groupe uniquement' }); break; }
        if (!state || !['on', 'off'].includes(state)) { await sock.sendMessage(remoteJid, { text: '❌ Utilise: .antidelete on/off' }); break; }
        const gs = initGroupSettings(remoteJid);
        gs.antidelete = state === 'on';
        await sock.sendMessage(remoteJid, { text: `✅ Antidelete: ${state.toUpperCase()}` });
        saveStoreKey('groupSettings');
        break;
      }
      case 'antiedit': {
        const state = args[0]?.toLowerCase();
        if (!isGroup) { await sock.sendMessage(remoteJid, { text: '❌ Groupe uniquement' }); break; }
        if (!state || !['on', 'off'].includes(state)) { await sock.sendMessage(remoteJid, { text: '❌ Utilise: .antiedit on/off' }); break; }
        const gs = initGroupSettings(remoteJid);
        gs.antiedit = state === 'on';
        await sock.sendMessage(remoteJid, { text: `✅ Antiedit: ${state.toUpperCase()}` });
        saveStoreKey('groupSettings');
        break;
      }
